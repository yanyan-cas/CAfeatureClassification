...
for j = 1 : CAEvol
    for i = 1 : size(extendTrain, 1)
       if j ==1
           output = CAtwoDFeatureFredkin(reshape(extendTrain(i,:), 128, 128)');
       else
           output =  CAtwoDFeatureFredkin(reshape(extendCATrain(i,:), 128, 128)');
       end
       extendCATrain(i,:) = reshape(output', 1, 128*128);
    end
    filename = sprintf('DataRuleFredkinEvolution%d', j);
...
for evolve = 1 : CAEvol
    ...
    filename = sprintf('DataRuleFredkinEvolution%d',  evolve);
...
filestorename = sprintf( 'exptest-kNNwithBinaryFredkin_%dth_CIFAR.mat',  evolve);
